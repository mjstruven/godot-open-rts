...
@onready var _tower_menu = find_child("TowerMenu")
...
func _try_showing_ability_menu(focused_units: Array):
	...
	elif first.is_in_group("towers") and first.is_constructed():
		_tower_menu.tower = first
		_tower_menu.show()
